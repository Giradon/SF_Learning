...
const delayAdd = (index, callback, error) => {
    setTimeout(() => {
        if (index > 10) {
            error(`index(${index})는 10보다 클 수 없습니다.`);
            return;
        }
...
const newDelayAdd = (idx) => {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (idx > 10) {
                reject(`index(${index})는 10보다 클 수 없습니다.`);
                return;
            }
            console.log('idx', idx);
            resolve(idx + 1);
 ...
 * 비동기 코드 내부의 resolve, reject가 실행되는 것과 상관없이 무조건 실행된다.
 */
newDelayAdd(9)
...
const wrap = async () => {
    ...
    try {
        ...
        /** finally 메서드나 구문 같은 경우에는
         * 비동기 코드 내부의 resolve, reject가 실행되는 것과 상관없이 항상 실행되는 구조이다.
         */
